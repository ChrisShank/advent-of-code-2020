function sumGroupAnswers(input, countAnswers) {
  return input
    .split('\n\n')
    .map(group => {
      const people = group.split('\n')

      const answerMap = people.reduce((acc, person) => {
      ...
      return countAnswers(answerMap, people)
      })
...
console.log(sumGroupAnswers(
  answers,
  (answerMap) => Object.keys(answerMap).length
))

console.log(sumGroupAnswers(
  answers,
  (answerMap, people) => Object.values(answerMap).filter(answer => answer === people.length).length
))