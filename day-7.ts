const ruleRegex = /([\w\s]+) bags contain (?:no other bags)?(.*)./
const bagRegex = /(\d+) ([\w\s]+) bags?/

function parseRules(rules) {
  return rules
    .split('\n')
    .reduce((acc, rule) => {
      const [_, color, bagsContained] = ruleRegex.exec(rule)

      const bags = bagsContained
        .split(', ')
        .filter(str => str.length > 0)
        .reduce((acc, bag) => {
          const [_, number, color] = bagRegex.exec(bag)
          acc[color] = number
          return acc
        }, {})
...
function doesBagContainAGoldBag(bag, bags, cache) {
  if (bag === 'shiny gold') {
    return false
  } else if (cache[bag]) {
    return true
  ...
  const bagsContained = bags[bag]
  
  cache[bag] = Object.keys(bagsContained)
    .some(color => color === 'shiny gold' || doesBagContainAGoldBag(color, bags, cache))

  return cache[bag]
}

function calculateNumberOfBagsContainingAGoldBag(rules) {
  const bags = parseRules(rules)
  const cache = {}

  return Object.keys(bags).reduce((acc, bag) => {
    return acc + (doesBagContainAGoldBag(bag, bags, cache) ? 1 : 0)
  }, 0)
}

function countContaingBags(bag, bags, cache) {
  ...
  cache[bag] = Object.entries(bagsContained)
    .reduce((acc, [color, number]) => acc + (number * (1 + countContaingBags(color, bags, cache))), 0)
  return cache[bag]
}

function calculateNumberOfBagsInAGoldBag(rules) {
  const bags = parseRules(rules)
  const cache = {}

  return countContaingBags('shiny gold', bags, cache)
}

const rules = `shiny purple bags contain 2 pale blue bags, 1 wavy fuchsia bag, 5 pale salmon bags.
...
console.log(calculateNumberOfBagsContainingAGoldBag(rules))
console.log(calculateNumberOfBagsInAGoldBag(rules))
