...
function areFieldsPresent(passport) {
  ...
  return fields.every(field => passport[field] !== undefined)
}
...
function areFieldsValid(passport) {
  const fields = {
    ...
    'hgt': height => {
      const [_, dimensions, unit] = /(\d+)(cm|in)/.exec(height) || []
      if (unit === 'cm') {
        return isWithinRange(dimensions, 150, 193)
      } else if (unit === 'in') {
        return isWithinRange(dimensions, 59, 76)
      }
      return false
    }, 
  ...
  return Object.entries(fields).every(([field, validate]) => validate(passport[field]))
}
    
function validatePassports(input, validator) {
  const passports = parsePassports(input)
  return passports.filter(validator).length
}

const passports = `byr:1985
...
eyr:2039 hgt:64
ecl:#ab45a8 byr:2009
iyr:2025 pid:182cm hcl:d1614a cid:clear`

console.log(validatePassports(passports, areFieldsPresent))
console.log(validatePassports(passports, areFieldsValid))