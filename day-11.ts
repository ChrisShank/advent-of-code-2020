function parseSeatingLayout(layout) {
  return layout
    .split('\n')
    .map(row => row.split(''))
}
...
function seatingChanged(layoutOne, layoutTwo) {
  for (let y = 0; y < layoutOne.length; y += 1) {
    for (let x = 0; x < layoutOne[y].length; x += 1) {
      if (layoutOne[y][x] !== layoutTwo[y][x]) {
        return true
      }
    }  
  }
...
function areNoOccupiedSeatsAdjacent(layout, x, y) {
  return [
    layout[y][x - 1], // left
    layout[y][x + 1], // right
    layout[y - 1]?.[x], // above 
    layout[y + 1]?.[x], // below
    layout[y - 1]?.[x - 1], // upper left 
    layout[y - 1]?.[x + 1], // upper right
    layout[y + 1]?.[x - 1], // lower left 
    layout[y + 1]?.[x + 1], // lower right
  ]
    .filter(cell => cell !== undefined)
    .every(cell => cell === 'L' || cell === '.')
}

function areFourAjacentSeatsOccupied(layout, x, y) {
  return [
    layout[y][x - 1], // left
    layout[y][x + 1], // right
    layout[y - 1]?.[x], // above 
...
function simulateSeatingA(layout) {
  let currentLayout = parseSeatingLayout(layout)
  let nextLayout
  ...
  do {
    ...
    nextLayout = currentLayout.map((row, y) => row.map((cell, x) => {
      if (cell === 'L' && areNoOccupiedSeatsAdjacent(currentLayout, x, y)) { // empty seat
        return '#'
      } else if (cell === '#' && areFourAjacentSeatsOccupied(currentLayout, x, y)) { // occupied seat
        return 'L'
      }

      return cell
    }))
  } while (seatingChanged(currentLayout, nextLayout))

  return currentLayout
    .flat()
    .reduce((acc, cell) => acc + (cell === '#' ? 1 : 0), 0)
}

function getFirstSeat(layout, x, y, [rise, run]) {
  let cell
  do {
    x += run
    y += rise
...
function areNoOccupiedSeats(layout, x, y) {
  return [
    getFirstSeat(layout, x, y, [0, -1]), // left
    getFirstSeat(layout, x, y, [0, 1]), // right
    getFirstSeat(layout, x, y, [-1, 0]), // above
    getFirstSeat(layout, x, y, [1, 0]), // below 
    getFirstSeat(layout, x, y, [-1, -1]), // upper left
    getFirstSeat(layout, x, y, [-1, 1]), // upper right
    getFirstSeat(layout, x, y, [1, -1]), // lower left
    getFirstSeat(layout, x, y, [1, 1]) // lower right
  ]
    .filter(cell => cell !== undefined)
    .every(cell => cell === 'L' || cell === '.')
}

function areFiveSeatsOccupied(layout, x, y) {
  return [
    getFirstSeat(layout, x, y, [0, -1]), // left
    getFirstSeat(layout, x, y, [0, 1]), // right
    getFirstSeat(layout, x, y, [-1, 0]), // above
    ...
    getFirstSeat(layout, x, y, [1, -1]), // lower left
    getFirstSeat(layout, x, y, [1, 1]) // lower right
  ].filter(cell => cell === '#').length >= 5
}

function simulateSeatingB(layout) {
  let currentLayout = parseSeatingLayout(layout)
  let nextLayout
  ...
  do {
    ...
    nextLayout = currentLayout.map((row, y) => row.map((cell, x) => {
      if (cell === 'L' && areNoOccupiedSeats(currentLayout, x, y)) { // empty seat
        return '#'
      } else if (cell === '#' && areFiveSeatsOccupied(currentLayout, x, y)) { // occupied seat
        return 'L'
      }
...
const layout = `LLLLLL.LLL.LLLL.LLLLLLLLL.LLLLLLLLLLLLLLLL.LLLLLL.LLL.LLLL.LLLLLLLL.LLLLLLL.LLLLLLLLLLLLL..L.LLLLLL
...
console.log(simulateSeatingA(layout))
console.log(simulateSeatingB(layout))